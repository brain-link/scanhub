...
function ExamTree({ setDataPath }: ExamTreeProps) {
    ...
    const { data: exams, refetch, isLoading, isError } = useQuery<Exam[], Error>({
        ...
        <Stack sx={{ minWidth: '300px' }}>
...

            {
                isLoading ? <CircularProgress /> : 
                    isError ? <AlertItem title='Error loading exams' type={Alerts.Danger} /> :
                        <Card sx={{p: 0}}>
                                    ...
                                    <Accordion key={`exam-${exam.id}`}>
                                        ...
                                        <AccordionSummary>
                                            ...
                                            <ListItemContent>
                                                ...
                                                <Typography level="body-xs">{ `Created: ${new Date(exam.datetime_created).toDateString()}` }</Typography>
                                                <Typography level="body-xs">{ `Updated: ${exam.datetime_updated ? new Date(exam.datetime_updated).toDateString() : '-'}` }</Typography>
                                            </ListItemContent>
                                        </AccordionSummary>
                                                        ...
                                                        <Accordion key={`procedure-${procedure.id}`}>
                                                            ...
                                                            <AccordionSummary>
                                                                ...
                                                                <ListItemContent>
                                                                    ...
                                                                    <Typography level="body-xs">{ `Created: ${new Date(procedure.datetime_created).toDateString()}` }</Typography>
                                                                    <Typography level="body-xs">{ `Updated: ${procedure.datetime_updated ? new Date(procedure.datetime_updated).toDateString() : '-'}` }</Typography>
                                                                </ListItemContent>
                                                            </AccordionSummary>
