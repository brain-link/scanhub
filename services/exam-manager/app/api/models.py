...
async def get_procedure_out(data: Exam) -> ProcedureOut:
...
async def get_exam_out(data: Exam) -> ExamOut:
    procedures = [get_procedure_out(procedure) for procedure in data.procedure] if hasattr(data, "procedure") else []
    return ExamOut(
        id=str(data.id),
