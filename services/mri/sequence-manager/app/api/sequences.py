...
async def create_sequence(payload: SequenceIn):
    for device_id in payload.devices_id:
        if not is_device_present(device_id):
            raise HTTPException(
                status_code=404, detail=f"Device with given id:{device_id} not found"
            )

    sequence_id = await db_manager.add_sequence(payload)
    response = {"id": sequence_id, **payload.dict()}
    ...
    if "devices_id" in update_data:
        for device_id in payload.devices_id:
            if not is_device_present(device_id):
                raise HTTPException(
                    status_code=404,
                    detail=f"Device with given id:{device_id} not found",
                )
