...
def headerFill(header_xml, **kwargs):
    ...
    field_mapping = {
        ...
        "relativeTablePosition": (
            header_xml.measurementInformation,
    ...
    for field, value in kwargs.items():
        ...
        if expected_type == "date":
            try:
                value = datetime.datetime.strptime(
                    value, "%Y-%m-%d"
                ).date()  # Konvertieren zu Date-Objekt
            except ValueError:
            ...
            try:
                value = datetime.datetime.strptime(
                    value, "%H:%M:%S"
                ).time()  # Konvertieren zu Time-Objekt
            except ValueError:
            ...
            if not (isinstance(value, (int, np.integer)) and 0 <= value <= 65535):
                print(
                    f"Fehler: '{field}' muss eine ganze Zahl im Bereich von 0 bis 65535 sein."
                )
                return false
        ...
        # "Special" datatypes #
            ...
            if (
                ...
                print(
                    f"Fehler: '{field}' muss eine Liste oder ein Array mit genau 3 Zahlen sein."
                )
                return false
            ...
            if value not in (
                ...
                print(
                    f"Fehler: '{field}' muss 'HFP', 'HFS', 'HFDR', 'HFDL', 'FFP', 'FFS', 'FFDR' oder 'FFDL' sein."
                )
                return false
        elif not isinstance(value, expected_type):
            print(
                f"Fehler: '{field}' erwartet {expected_type.__name__}, aber {type(value).__name__} erhalten."
            )
            return false
...
def fill_mrd_template(
    ...
    )  # <- with "import datetime"  YYYY-MM-DD <- "2025-03-18"
    ...
    # === measurementInformation ===
    header_xml = headerFill(
        header_xml, measurementID=measurement_id, patientPosition=patient_position
    )
    ...
    header_xml = headerFill(
        ...
        receiverChannels=n_coil_available,
    )

    # === experimentalConditions ===
    header_xml = headerFill(
        header_xml, H1resonanceFrequency_Hz=h1_resonance_frequency_hz
    )
    ...
    for i in range(mrd_template.number_of_acquisitions()):
        ...
        setattr(
            current_acq_head, "available_channels", ctypes.c_uint16(n_coil_available)
        )

        # Set encoding directions
        for fieldname in ["read_dir", "phase_dir", "slice_dir"]:
            tmp_dir_result_np_array = np.dot(
                slice_orientation_matrix,
                np.frombuffer(getattr(current_acq_head, fieldname), dtype=np.float32),
            )
        ...
        # Set Slice position
        position_result_np_array = np.add(
            np.frombuffer(getattr(current_acq_head, "position"), dtype=np.float32),
            position_shift,
        )
        ...
        table_position_result_ctypes_array = (ctypes.c_float * 3)(
            *patient_table_position
        )
        setattr(
            current_acq_head,
            "patient_table_position",
            table_position_result_ctypes_array,
        )
        ...
        try:
            ...
            print(
                f"Error: Mismatch in rawData size at acquisition {i}. Skipping acquisition."
            )
            break
...
def main():
    ...
    slice_orientation_matrix = np.array(
        [[1, 0, 0], [0, 0, -1], [0, 1, 0]], dtype=np.float32
    )
    gradient_scaling_factors = np.array(
