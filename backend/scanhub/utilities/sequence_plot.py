...
# Generate the plot data object
def get_sequence_data(sequence_path: str):

    seq_obj = Sequence(system=system)
    seq_obj.read(sequence_path)
    data = pulseq_to_dict(seq_obj)
    traces = []

    for idx, key in enumerate(data.keys()):
        traces.append(
            PlotData(
                x=list(data[key]["time"]),
                y=list(data[key]["values"]),
                type="scatter",
                mode="lines",
                name=data[key]["name"],
                yaxis=f"y{len(data.keys())-idx}"
                # marker=dict( color=clr.plot[idx] )
            )
        )

    return parse_obj_as(List[PlotData], traces)
...
plot = get_sequence_data('/scanhub/scanhub/ressources/epi_pypulseq.seq')